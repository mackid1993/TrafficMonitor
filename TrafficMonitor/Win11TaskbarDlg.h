...
private:
    ...
    //"保持原位"的截止时间。预留区域一直建不起来时（占位图标没能被标记为始终显示、
    //用户在设置里把它们关掉、外壳拒绝添加等），如果一直保持原位不动，
    //窗口就会永远停在托盘左边缘那个位置上，而每多显示一个占位图标托盘就往左长一截，
    //最后窗口正好压在真正的托盘图标上面，且永远不会自己让开。
    //超过这个时限就放弃保持，回到普通的摆放方式；预留区域后来要是建成了，照样会被用上。
    //Deadline for holding position. If the region never materialises - placeholders not marked
    //always-visible, the user switching them off in Settings, the shell refusing adds - holding
    //forever leaves the window parked at the tray's left edge while every placeholder that does
    //appear grows the tray leftwards over it, until it sits on top of real tray icons and never
    //moves away. Past this deadline, stop holding and place normally; if the region does turn up
    //later it is still used.
    static constexpr ULONGLONG HOLD_TIMEOUT = 15000;
