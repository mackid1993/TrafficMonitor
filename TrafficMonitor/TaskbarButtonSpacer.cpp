...
GUID CTaskbarTrayReserve::MakeIconGuid(int index)
{
    ...
    //这一组GUID曾经被外部删掉过对应的注册表项，导致外壳的内存状态和注册表不一致：
    //外壳仍然认得这些身份，却再也不会为它们回写注册表项，占位图标也就永远无法显示。
    //换一组全新的GUID可以绕开这些已经废掉的身份。
    //An earlier build deleted these entries' registry keys from outside the shell, leaving its
    //in-memory state and the registry inconsistent: the shell still recognises those identities
    //but will never write their keys back, so those placeholders can never be shown again.
    //A fresh block of GUIDs sidesteps the poisoned ones.
